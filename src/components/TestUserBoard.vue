<template>
  <div class="user-board-wrapper">
    ...
    <button v-if="user.id === currentUser.id" class="btn btn-user-info" type="button" data-bs-toggle="modal"
      v-bind:data-bs-target="'#testModalUserInfo'">編輯個人資料</button>
    ...
    <TestModalUserInfo v-bind:propsUser="currentUser" v-on:after-put-userInfo="afterPutUserInfo" />
  </div>
</template>

<script>
import ModalUserInfo from './ModalUserInfo.vue'
import TestModalUserInfo from './TestModalUserInfo.vue'
export default {
  components: {
    ModalUserInfo,
    TestModalUserInfo
